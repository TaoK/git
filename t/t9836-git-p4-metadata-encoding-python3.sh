...
python_3_exists=$(/usr/bin/python3 -V 2>&1)
if test "$python_major_version" = '2' && test "$python_3_exists"
then
	mkdir temp_python
	export PATH="$(pwd)/temp_python:$PATH"
	ln -s /usr/bin/python3 temp_python/python
fi

python_major_version=$(python -V 2>&1 | cut -c  8)
if test "$python_major_version" = '2'
then
	skip_all='skipping python3-specific git p4 tests; python3 not available'
	test_done
fi
